package it.unibo.authsim.library.cryptography.algorithm.hash
...
class HashFunctionTest extends AnyWordSpec with Matchers with BeforeAndAfter {
  ...
  "A password" when {
    "hashed with the SHA1 function" should{
      "use as algorithm" in{
        sha1.name shouldBe "SHA1"
      }
  ...
  "A password" when {
    "hashed with the SHA256 function" should {
      "use as algorithm" in{
        sha256.name shouldBe "SHA256"
      }
  ...
  "A password" when {
    "hashed with the SHA384 fucntion" should{
      "use as algorithm" in{
        sha384.name shouldBe "SHA384"
      }
      "return the hashed value" in {
  ...
  "A password" when {
    "hashed with MD5 funciton" should {
      "use as algorithm" in{
