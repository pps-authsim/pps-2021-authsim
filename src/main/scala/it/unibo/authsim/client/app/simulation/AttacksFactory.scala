...
class AttacksFactory(private val userProvider: UserProvider, private val logger: StatisticsConsumer):
   ...
   */
  def bruteForceLowers(): AttackBuilder = new BruteForceAttackBuilder() against userProvider usingAlphabet defaultAlphabets.lowers logTo logger maximumLength 6
   ...
   */
  def bruteForceLetters(): AttackBuilder = new BruteForceAttackBuilder() against userProvider usingAlphabet (defaultAlphabets.lowers and defaultAlphabets.uppers) maximumLength 10 logTo logger timeout Duration(120, TimeUnit.SECONDS)
   ...
   */
  def bruteForceAll(): AttackBuilder = new BruteForceAttackBuilder() against userProvider usingAlphabet defaultAlphabets.alphanumericsymbols maximumLength 16 logTo logger timeout Duration(600, TimeUnit.SECONDS)
