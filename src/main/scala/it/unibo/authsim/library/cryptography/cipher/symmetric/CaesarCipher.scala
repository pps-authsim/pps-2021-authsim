...
import it.unibo.authsim.library.cryptography.algorithm.symmetric.Caesar
import it.unibo.authsim.library.cryptography.cipher.{Cipher, SymmetricCipher}
...
object CaesarCipher:
  ...
  case class BasicCaesarCipher() extends Cipher with SymmetricCipher:
     ...
     */
    override val algorithm : Caesar = Caesar()
     ...
     * @param rotation : shift to be applied
     * @tparam A : generic parameter for the password
     * @tparam B : generic parameter for the secret, if it is not an integer or cannot be cast to one in a reasonable way it is substitued with the default value, which is 0
